...
def load_datashader_maps(experiment, plot_width):
    ...
    fh = folderpath+'datashader/'+ experiment+'_tracks_classes' +'_'+str(plot_width)+ '_v2.pkl'
...
def compute_similarity_map(track_dict, classes_to_compare=['scouts','herd']):
    '''
...
def compute_similarity_map_all(data, inputs):
    ...
    result = {}
    for experiment in experiments:
        result[experiment],_,_ = compute_similarity_map(data[experiment][0], classes_to_compare=['scouts','herd'])
