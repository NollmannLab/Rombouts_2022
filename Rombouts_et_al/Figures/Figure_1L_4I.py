...
# -*- coding: utf-8 -*-
"""
Created on Thu Feb  3 14:13:13 2022
...
histograms of MSD alpha exponents for all tracks.
...
def extractDirectionalityFromData(data, inputs):
    exp_WT = getInfoFromListOfDict(inputs, 'exp_WT')
    exp_aS = getInfoFromListOfDict(inputs, 'exp_aS')
    exp_As = getInfoFromListOfDict(inputs, 'exp_As')
    ...
    for strain in range(len(strains)):
        for experiment in strains[strain]:
            print("extracting directionality from ", experiment)
            data_strain = data[data['Experiment'] == experiment]
            ind = filterDataRange(data_strain, inputs)
            df_n=pd.concat([df_n,   data_strain.loc[ind, 'MSD_n_5frames'].reset_index(drop=True)],axis=1,ignore_index=True)
    # Give column names based on strains list of names
    df_n.columns = [item for sublist in strains for item in sublist]
    return df_n

def computeDirectionalityDist(df_n, inputs):
    exp_WT = getInfoFromListOfDict(inputs, 'exp_WT')
    exp_aS = getInfoFromListOfDict(inputs, 'exp_aS')
    exp_As = getInfoFromListOfDict(inputs, 'exp_As')
    ...
    # bins = np.linspace(-0.5, 2.1, 25)
    # for strain in range(len(strains)):
    #     for experiment in strains[strain]:
    #         df_dist[experiment], _, _ = plt.hist(df_n[experiment], bins=bins, density=True)
    ...
    # plt.xlabel("Directionality", fontsize = 18)
    # plt.gca().set_xlim(-0.5,2)
    # plt.ylabel("Normalized density", fontsize = 18)
    # plt.legend(loc="upper left")
    
    # -- Compute distributions using Kernel
        
    df_dist = pd.DataFrame()
    bins = np.linspace(-0.5, 2.1, 200)[:, np.newaxis]  
    bins_hist = np.linspace(-0.5, 2.1, 50)
    for strain in range(len(strains)):
        fig, ax = plt.subplots(figsize=(8,8))
        for experiment in strains[strain]:
            X = df_n[experiment].unique()
            X = X[~np.isnan(X)][:,None]
            kde = KernelDensity(kernel='gaussian', bandwidth=0.2).fit(X) # Fit the Kernel Density model on the data.
            log_dens = kde.score_samples(bins)    # Evaluate the log density model on the data.
            df_dist[experiment] =np.exp(log_dens)/np.exp(log_dens).sum()
            
            plt.plot(bins, df_dist[experiment], label=experiment)
            # plt.hist(X, bins=bins_hist, histtype=u'step', density=True, label=experiment)
        
        plt.xlabel("Directionality", fontsize = 36)
        ax.set_xlim(-0.5,2)
        ax.set_ylim([0,None])
    ...
    fig, ax = plt.subplots(figsize=(8,8))
    bins = np.linspace(-0.5, 2.1, 200)
    lw = 4
    for strain_names in strains_names:
        Mean = df_dist[locals()[strain_names]].mean(axis=1)
        Std = df_dist[locals()[strain_names]].std(axis=1)
        # ax.errorbar(bins[:, 0], Mean, yerr=Std, errorevery=5, lw=lw, linestyle='-',capsize=4,elinewidth=1, label=strain_names)
        ax.plot(bins, Mean, label=strain_names)
        ax.fill_between(bins, Mean - Std, Mean + Std, alpha=0.2)
    plt.xlabel("Directionality", fontsize = 36)
    ax.set_xlim(-0.5,2)
    ax.set_ylim([0,None])
    plt.ylabel("Normalized density", fontsize = 36)
...
if __name__ == '__main__':
    ...
    # Filter data range and extract directionality
    df_n = extractDirectionalityFromData(data, inputs)
    
    # Computes directionality distributions
    computeDirectionalityDist(df_n, inputs)
